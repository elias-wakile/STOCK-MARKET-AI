...
class PortFolio:
    ...
    def linear_reward(self):
        """
        This function executes the predictions of the Extrapolation Model
        :return: The actions that do and reward vector of the same format
        """
        ...
        for index in self.stock_indices.keys():
            ...
            if num_of_stocks > 0:
                ...
                print(f"Sold {-num_of_stocks} stock(s) of {stock_name}: "
                      f"{self.stocks[stock_name].last_low_price}$ per stock, and make a profit of {profit}$.")
    ...
    def sort_buy(self, stock_predictions):
        """
        This function sorts the stocks by their price so that the program can buy as many stocks as it wants
        :param stock_predictions: the prediction of the model on how mach stock to buy
        """
        but_dic = {}
    ...
    def action(self, stock_predictions):
        """
        This function executes the predictions of the NeuralNetwork Model
            ...
            for index in stock_predictions[num_of_stocks]:
                ...
                if num_of_stocks > 0:
                    ...
                    for j in range(num_of_stocks):
                        self.rewards_dict[stock_name].append(trade_result)
                        self.balance -= self.stocks[stock_name].daily_lowest
                elif num_of_stocks < 0:
